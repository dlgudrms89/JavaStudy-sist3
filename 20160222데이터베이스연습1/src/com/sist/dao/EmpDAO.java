...
public class EmpDAO {
   ...
   private final String URL="jdbc:oracle:thin:@211.238.142.39:1521:ORCL";
   //드라이버 설치
   public EmpDAO()
   {
	   try
   ...
   public EmpDTO empDetailData(int empno)
   {
	   ...
	   try
	   {
		   //연결
		   getConnection();
		   String sql="SELECT * FROM emp WHERE empno=?"; // ? => 데이터 값 보낼 때
		   ps=conn.prepareStatement(sql);
		   ps.setInt(1, empno);//첫번째 물음표에 empno를 넣는다
		   ResultSet rs=ps.executeQuery();
		   rs.next();
		   d.setEmpno(rs.getInt(1));
		   d.setEname(rs.getString(2));
   ...
   public ArrayList<EmpDTO> empFindData(String column,String data)
   {
	   ...
	   try
	   {
		   getConnection();
		   String sql="SELECT empno,ename,job,hiredate,deptno "
		   		+ "FROM emp WHERE "+column+" LIKE '%'||?||'%'"; // LIKE가 SQL과 다름
		   ps=conn.prepareStatement(sql); //전송
		   ps.setString(1, data.toUpperCase()); //setString:스트링으로 인식, toUpperCase():대문자로
		   // 실행후에 결과값 받기
		   ResultSet rs=ps.executeQuery();
		   while(rs.next())
		   {
   ...
   public ArrayList<ZipcodeDTO> postFind(String dong)
   {
	   ...
	   {
		   getConnection();
		   String sql="SELECT zipcode,sido,gugun,dong,NVL(bunji,' ') FROM zipcode " //NVL은 null값을 제거할 때 씀
		   		+ "WHERE dong LIKE '%'||?||'%'";
		   ps=conn.prepareStatement(sql);
		   ps.setString(1, dong);//1은 물음표 순서
		   ResultSet rs=ps.executeQuery();
		   while(rs.next())
		   {
